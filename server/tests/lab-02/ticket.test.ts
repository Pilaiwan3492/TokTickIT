...
describe("Ticket API Contract Tests (Lab 2)", () => {
  ...
  test("✓ Should filter and return records that match search query in ticketNo, summary, or description", async () => {
    ...
    const res = await request(app).get(
      `/api/v1/tickets?requesterId=${validRequesterId}&search=${encodeURIComponent(
        query
      )}`
    );
  ...
  test("✓ Should return 200 when filtering by existing inactive categoryId", async () => {
    ...
    const res = await request(app).get(
      `/api/v1/tickets?requesterId=${validRequesterId}&categoryId=${inactiveCategory.id}`
    );
  ...
  test("❌ Should return 403 FORBIDDEN when a different requester accesses another requester's ticket", async () => {
    const prisma = getPrisma();
    ...
    const differentRequester = await prisma.requesterUser.findFirst({
      where: {
        id: {
          not: validRequesterId,
        },
        isActive: true,
      },
    });

    if (!ownerTicket || !differentRequester) {
      return;
    }

    const res = await request(app).get(
      `/api/v1/tickets/${ownerTicket.id}?requesterId=${differentRequester.id}`
    );
  ...
  test("❌ Should return 404 TICKET_NOT_FOUND when the requested ticket does not exist", async () => {
    const nonExistentTicketId =
  ...
  // ---------------------------------------------------------
  // 3. Reference & Ownership Validations (400 INVALID_REFERENCE)
  // ---------------------------------------------------------
  ...
  test("🔴 Should return 400 INVALID_REFERENCE for nonexistent/inactive requesterId (e.g. 9999)", async () => {
    const res = await request(app).get(
      "/api/v1/tickets?requesterId=9999"
    );
  ...
  test("🔴 Should return 400 INVALID_REFERENCE for nonexistent categoryId (e.g. 9999)", async () => {
    const res = await request(app).get(
      `/api/v1/tickets?requesterId=${validRequesterId}&categoryId=9999`
    );
  ...
  test("✓ Should allow valid limit values (10, 20, 50)", async () => {
    for (const limit of [10, 20, 50]) {
      const res = await request(app).get(
        `/api/v1/tickets?requesterId=${validRequesterId}&limit=${limit}`
      );
  ...
  test("🔴 Should return 400 INVALID_QUERY for non-integer page (e.g. page=abc)", async () => {
    const res = await request(app).get(
      `/api/v1/tickets?requesterId=${validRequesterId}&page=abc`
    );
  ...
  test("🔴 Should return 400 INVALID_QUERY for invalid sort parameter (e.g. sort=summary_asc)", async () => {
    const res = await request(app).get(
      `/api/v1/tickets?requesterId=${validRequesterId}&sort=summary_asc`
    );
  ...
  test("✓ Should allow Whitelisted sort options", async () => {
    ...
    for (const sort of validSorts) {
      const res = await request(app).get(
        `/api/v1/tickets?requesterId=${validRequesterId}&sort=${sort}`
      );
  ...
  test("🔴 Should return 400 INVALID_QUERY for invalid priority filter", async () => {
    const res = await request(app).get(
      `/api/v1/tickets?requesterId=${validRequesterId}&priority=SUPER_HIGH`
    );
  ...
  test("🔴 Should return 400 INVALID_QUERY for invalid status filter", async () => {
    const res = await request(app).get(
      `/api/v1/tickets?requesterId=${validRequesterId}&status=UNKNOWN_STATUS`
    );
  ...
  test("🔴 Should return 400 INVALID_QUERY for non-integer categoryId", async () => {
    const res = await request(app).get(
      `/api/v1/tickets?requesterId=${validRequesterId}&categoryId=invalid_id`
    );
