...
| **TEST-API-05** | API | AC-04, BR-05 | Uploading an unsupported attachment type (EXE) or an attachment larger than 5 MiB | Returns HTTP 415 Unsupported Media Type for unsupported file type and HTTP 413 Payload Too Large for oversized file | `tests/api/attachment-upload.test.ts` | `[Pending]` |
...
| **TEST-API-08** | API | AC-04, BR-05 | Uploading a valid attachment (JPG/JPEG/PNG/WEBP/PDF <= 5 MiB / 5,242,880 bytes) | Creates attachment metadata and returns HTTP 201 Created | `tests/api/attachment-upload.test.ts` | `[Pending]` |
