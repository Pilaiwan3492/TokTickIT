...
| **TEST-API-05** | API | AC-04, BR-05 | Uploading invalid attachment type (EXE) or >5MB[cite: 1] | Returns HTTP 400 Bad Request with field validation error[cite: 1] | `tests/api/attachment-upload.test.ts` | `[Pending]` |
| **TEST-API-06** | API | AC-05, BR-07 | Soft removing attachment via `DELETE /api/v1/attachments/:id`[cite: 1] | Sets `removedAt` & `removalReason`; download link returns HTTP 404 Not Found[cite: 1] | `tests/api/attachment-soft-remove.test.ts` | `[Pending]` |
